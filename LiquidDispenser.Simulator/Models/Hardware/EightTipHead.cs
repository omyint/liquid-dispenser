...
public class EightTipHead : IPipetteHead
{
    ...
    public async Task AspirateAsync(double volumePerTip, CancellationToken cancellationToken = default)
    {
        if (!HasTips)
            throw new InvalidOperationException("Cannot aspirate without tips.");
        await Task.Delay(300, cancellationToken); // Simulate plunger movement
    ...
    public async Task DispenseAsync(double volumePerTip, CancellationToken cancellationToken = default)
    {
        if (!HasTips)
    ...
    public async Task DropTipsAsync(CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
    ...
    public async Task PickupTipsAsync(CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        await Task.Delay(400, cancellationToken); // Simulating pressing into a tip rack
        HasTips = true;
    }
}
